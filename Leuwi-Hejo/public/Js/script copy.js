...
function form(formId, fieldSettings) {
  ...
  form.addEventListener("submit", function (event) {
    ...
    let textLahan =
      fields.lahan.options[fields.jenisKendaraan.selectedIndex].text;
    let kendaraan =
      fields.jenisKendaraan.options[fields.jenisKendaraan.selectedIndex].text;
    let jumlahKendaraan = "";
    let tanggal = fields.tanggal.value.trim();

    if (fields.jenisKendaraan.value === "Motor") {
      let motorSelected =
        document.getElementById("Motor").options[
          document.getElementById("Motor").selectedIndex
        ];
      jumlahKendaraan = motorSelected.textContent;
    } else if (fields.jenisKendaraan.value === "Mobil") {
      let mobilSelected =
        document.getElementById("Mobil").options[
          document.getElementById("Mobil").selectedIndex
        ];
    ...
    if (perlengkapanCheckbox.checked) {
      const itemCheckboxes = perlengkapanList.querySelectorAll(
...
function initFormReguler(formId, fieldSettings) {
  ...
  form.addEventListener("submit", function (event) {
    ...
    const waLink = `https://wa.me/${nomorPemilik}?text=${encodeURIComponent(
      pesan
    )}`;
...
function initFormCamping(formId, fieldSettings) {
  ...
  form.addEventListener("submit", function (event) {
    ...
    let kendaraan = fields.jenisKendaraan
      ? fields.jenisKendaraan.options[fields.jenisKendaraan.selectedIndex].text
      : "";
    let tanggal = fields.tanggal.value.trim();
...
function initFormTrakking(formId, fieldSettings) {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (!validateNomor(nomor)) {
      const errorMessage =
        "Nomor WhatsApp tidak valid. Masukkan format yang benar, contoh: 08xxxx atau 628xxx";
      alert(errorMessage);
      return;
    }
