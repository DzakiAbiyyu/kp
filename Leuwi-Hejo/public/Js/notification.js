function loadNotifications() {
    $.getJSON(baseUrl + "/admin/notifications/ajax/get", function (data) {
        ...
        if (!data.notifications || data.notifications.length === 0) {
            container.html('<div class="text-center small text-muted py-3">Tidak ada notifikasi</div>');
            badge.text('');
            return;
        }
        ...
        data.notifications.forEach(function (n) {
            ...
            const notifHtml = `
<a class="dropdown-item d-flex align-items-center notif-item ${isUnread ? 'bg-light notif-unread' : ''}" 
   href="${n.link ?? '#'}" 
   data-id="${n.notif_id}">
...
$(document).ready(function () {
    ...
    $(document).on('click', '.notif-item', function (e) {
        ...
        $.post(baseUrl + "/admin/notifications/mark-read", {
            id: id
        }, function (response) {
            if (response.success) {
